...
**e) Cache local:** registre cada publicação no seu controle local de prazos (planilha, CRM ou um arquivo JSONL simples com processo/teor/data fatal/cliente/tipo), com dedupe por nº de processo + teor. A partir daí, suas consultas de prazos do dia/da semana (e o agente `secretaria-juridica`) respondem **instantaneamente, sem re-chamar a API**. No criminal, calcule a data fatal pelas regras de contagem de prazo do CPP.
